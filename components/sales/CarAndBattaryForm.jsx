...
import SearchableSelectModel from '../general/SearchableSelectModel';
...
const CarAndBattaryForm = ({setCarData}) => {
    ...
    console.log("🚀 ~ CarAndBattaryForm ~ currentBrandModels:", currentBrandModels)
    const [currentModel, setCurrentModel] = useState("");
    console.log("🚀 ~ CarAndBattaryForm ~ currentModel:", currentModel)
    ...
    // filter brand models and set current model
    useEffect(() => {
        if (currentBrand && allCarsData.length) {
            const filteredModels = allCarsData.filter(
                carData => carData[0] === currentBrand
            )
            setCurrentBrandModels(filteredModels)
            setCurrentModel(filteredModels[0])
    ...
    useEffect(() => {
        if (currentModel.length) {
            const yearsRange = currentModel[2].split("~")
    ...
    useEffect(() => {
        if (currentModel) {
            setCarData(
                {brand: currentBrand, model: currentModel[1], year: currentYear, standardSize, upgradeSize}
            )
        }
    ...
    const currentModelsOptions = currentBrandModels.map(
        model => <option key={uuidv4()} value={model}>{`${model[1]} (${model[2]})`}</option>
    )
    ...
    return (
        <form
            ...
            <div className='formRow'>
                ...
                <SearchableSelectStr
                    orgignalList={carBrands}
                    currentItem={currentBrand}
                    setCurrentItem={setCurrentBrand}
                    open={open}/>
            ...
            <div className='formRow'>
                ...
                <SearchableSelectModel
                    orgignalList={currentBrandModels}
                    currentItem={currentModel}
                    setCurrentItem={setCurrentModel}
                    open={open}/>
                <div className='w-1/3'></div>
            </div>
            {/* year */}
            <div
                ...
                <SearchableSelectStr
                    orgignalList={modelYears}
                    currentItem={currentYear}
                    setCurrentItem={setcurrentYear}
                    open={open}/> {/* <select
                    name="year"
                    id="year"
                    className='select'
